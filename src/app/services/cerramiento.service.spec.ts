...
import { CerramientoService } from './cerramiento.service';
import { Cerramiento } from '../models/cerramiento';
import { AUTH } from '../auth/auth.constants';

describe('CerramientoService', () => {
  let service: CerramientoService;
  let httpMock: HttpTestingController;
  const api = `${AUTH.API}/cerramientos`;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(CerramientoService);
  ...
  it('getAll() hace GET a la URL base con los filtros como query params', () => {
    service.getAll({ nombre: 'Vallado' }).subscribe();

    const req = httpMock.expectOne(request => request.url === api && request.method === 'GET');
    expect(req.request.params.get('nombre')).toBe('Vallado');
  ...
  it('addRegistro() hace POST con el cerramiento en el body', () => {
    const datos = {
      nombre: 'Vallado',
      descripcion: 'Vallado perimetral',
      activo: true
    } as Cerramiento;
