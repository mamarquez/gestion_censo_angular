...
describe('TipoInstalacionService', () => {
  let service: TipoInstalacionService;
  let httpMock: HttpTestingController;
  const api = `${AUTH.API}/tiposinstalaciones`;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(TipoInstalacionService);
  ...
  it('getAll() hace GET a la URL base con los filtros como query params', () => {
    service.getAll({ nombre: 'Piscina' }).subscribe();

    const req = httpMock.expectOne(request => request.url === api && request.method === 'GET');
    expect(req.request.params.get('nombre')).toBe('Piscina');
  ...
  it('addRegistro() hace POST con el tipo de instalación en el body', () => {
    const datos: TipoInstalacion = {
      nombre: 'Piscina',
      descripcion: 'Piscina cubierta',
      valor: 'piscina',
      activo: true
    };
