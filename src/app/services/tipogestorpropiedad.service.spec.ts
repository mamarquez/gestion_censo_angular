...
import { TipoGestorPropiedadService } from './tipogestorpropiedad.service';
import { TipoGestorPropiedad } from '../models/TipoGestorPropiedad';
import { AUTH } from '../auth/auth.constants';

describe('TipoGestorPropiedadService', () => {
  let service: TipoGestorPropiedadService;
  let httpMock: HttpTestingController;
  const api = `${AUTH.API}/tiposgestorespropiedades`;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(TipoGestorPropiedadService);
  ...
  it('getAll() hace GET a la URL base con los filtros como query params', () => {
    service.getAll({ nombre: 'Público' }).subscribe();

    const req = httpMock.expectOne(request => request.url === api && request.method === 'GET');
    expect(req.request.params.get('nombre')).toBe('Público');
  ...
  it('addRegistro() hace POST con el tipo de gestor en el body', () => {
