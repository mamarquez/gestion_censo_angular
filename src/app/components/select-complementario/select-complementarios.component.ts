...
@Component({
  standalone: true,
  selector: 'app-select-complementarios',
  imports: [CommonModule, FormsModule, SelectModule],
  templateUrl: './select-complementarios.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SelectComplementariosComponent),
      multi: true
    }
  ]
})
export class SelectComplementariosComponent implements ControlValueAccessor, OnInit {
  private readonly service = inject(EspacioComplementarioService);
  private readonly destroyRef = inject(DestroyRef);

  espaciosComplementarios = signal<EspacioComplementario[]>([]);
  ...
  ngOnInit(): void {
    this.cargarEspaciosComplementarios();
  }

  private cargarEspaciosComplementarios(): void {
    this.service.getAll(this.filtros)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
          this.espaciosComplementarios.set(response.data || []);
