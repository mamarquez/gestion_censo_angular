...
describe('TiposGestoresPropiedadesComponent', () => {
  ...
  const crearTiposGestoresPropiedadesMock = (): TipoGestorPropiedad[] => [
    { id: 1, nombre: 'Público', mostrar: 'Público', activo: true },
    { id: 2, nombre: 'Privado', mostrar: 'Privado', activo: false }
  ];
  let tiposGestoresPropiedadesMock: TipoGestorPropiedad[];
  ...
  beforeEach(async () => {
    tiposGestoresPropiedadesMock = crearTiposGestoresPropiedadesMock();
    ...
    serviceSpy.getAll.and.returnValue(of(respuesta(tiposGestoresPropiedadesMock)));
  ...
  it('se crea correctamente y carga el listado en ngOnInit', () => {
    ...
    expect(component.tiposGestoresPropiedades).toHaveSize(2);
  ...
  it('cambiarEstado() invierte el estado activo del tipo gestor propiedad afectado', () => {
    fixture.detectChanges();
  ...
  it('confirmarBorrado() delega en DialogService con el nombre del tipo gestor propiedad', () => {
    fixture.detectChanges();
    const tipoGestorPropiedad = component.tiposGestoresPropiedades.find(t => t.id === 1)!;

    component.confirmarBorrado(tipoGestorPropiedad);
  ...
  it('abrirModal() limpia el tipo gestor propiedad seleccionado y abre el modal', () => {
    fixture.detectChanges();
    component.tipoGestorPropiedad = tiposGestoresPropiedadesMock[0];
  ...
  it('editar() carga el tipo gestor propiedad y abre el modal', () => {
    fixture.detectChanges();
    serviceSpy.get.and.returnValue(of(respuesta(tiposGestoresPropiedadesMock[0])));

    component.editar('1');

    expect(serviceSpy.get).toHaveBeenCalledWith('1');
    expect(component.tipoGestorPropiedad).toEqual(tiposGestoresPropiedadesMock[0]);
    expect(component.modalVisible).toBeTrue();
  });
