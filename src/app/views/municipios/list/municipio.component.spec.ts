...
describe('MunicipioComponent', () => {
  ...
  const crearMunicipiosMock = (): Municipio[] => [
    { id: 1, cpro: '11', cmun: '001', dc: '1', nombre: 'Cádiz', activo: true },
    { id: 2, cpro: '11', cmun: '002', dc: '2', nombre: 'Jerez', activo: false }
  ];
  let municipiosMock: Municipio[];
  ...
  beforeEach(async () => {
    municipiosMock = crearMunicipiosMock();
    serviceSpy = jasmine.createSpyObj('MunicipioService', [
    ...
    serviceSpy.getAll.and.returnValue(of(respuesta(municipiosMock)));

    await TestBed.configureTestingModule({
      imports: [MunicipioComponent],
      providers: [
        { provide: MunicipioService, useValue: serviceSpy },
  ...
  it('cambiarEstado() invierte el estado activo del municipio afectado', () => {
    fixture.detectChanges();
    ...
    expect(component.municipios.find(m => m.id === 1)?.activo).toBeFalse();
  });
  ...
  it('confirmarBorrado() delega en DialogService con el nombre del municipio', () => {
    fixture.detectChanges();
    const municipio = component.municipios.find(m => m.id === 1)!;

    component.confirmarBorrado(municipio);
  ...
  it('confirmarBorrado() -> onAccept borra el registro y lo quita del listado', () => {
    ...
    expect(component.municipios.find(m => m.id === 1)).toBeUndefined();
  });
  ...
  it('abrirModal() limpia el municipio seleccionado y abre el modal', () => {
    fixture.detectChanges();
    component.municipio = municipiosMock[0];

    component.abrirModal();

    expect(component.municipio).toBeNull();
    expect(component.modalVisible).toBeTrue();
  });

  it('editar() carga el municipio y abre el modal', () => {
    fixture.detectChanges();
    serviceSpy.get.and.returnValue(of(respuesta(municipiosMock[0])));

    component.editar('1');

    expect(serviceSpy.get).toHaveBeenCalledWith('1');
    expect(component.municipio).toEqual(municipiosMock[0]);
    expect(component.modalVisible).toBeTrue();
  });
  ...
  it('guardar() sin id llama a addRegistro() y recarga el listado', () => {
    ...
    component.guardar({ cpro: '11', cmun: '003', dc: '3', nombre: 'Rota', activo: true } as Municipio);

    expect(serviceSpy.addRegistro).toHaveBeenCalledWith(jasmine.objectContaining({ nombre: 'Rota' }));
  ...
  it('guardar() con id llama a updateRegistro() y recarga el listado', () => {
    ...
    component.guardar({ id: 1, cpro: '11', cmun: '001', dc: '1', nombre: 'Cádiz capital', activo: true } as Municipio);
  ...
  it('guardar() notifica error si addRegistro falla', () => {
    ...
    component.guardar({ nombre: 'Rota', activo: true } as Municipio);
  ...
  it('guardar() notifica error si updateRegistro falla', () => {
    ...
    component.guardar({ id: 1, nombre: 'Cádiz capital', activo: true } as Municipio);
